...
ak_shared_rbind_sums <- ak_total_shared_rbind

ak_shared_rbind_sums <- ak_shared_rbind_sums %>% group_by(Sample) %>% summarise(count=length(unique(ASV)), abund=sum(rel_abun))
