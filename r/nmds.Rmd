...
# make a copy of the merged OTU table (from ak_split). Before transposing, hosts/soil are columns and ASVs are rows. 
ak_nmds_mat <- t(ak_skookag_mat)

# convert to data frame
ak_nmds_mat <- as.data.frame(ak_nmds_mat)

# change host names and soil from row name to column so ggplot can recognize later
# add a column with the habitat name for later use in ggplot
...
# make a copy of the merged OTU table. Before transposing, hosts/soil are columns and ASVs are rows. 
...
nmds_plot <- ggplot(data_scores, aes(x = NMDS1, y = NMDS2)) + 
    ...
    annotate("text", label="stress=0.108", x=0.8, y=-0.7, colour="black") +
    labs(x = "NMDS1", y = "NMDS2", shape = "Habitat", colour = "Sample") +
...
# take the OTU table from the physeq object subsetted by sample, merged by plot
soil_nmds_mat <- as.data.frame(otu_table(merged_soil_plot))

# make plot names into a column
soil_nmds_mat <- rownames_to_column(soil_nmds_mat, 'Plot')

# extract first and second characters from the strings in column 1 ("AK" or "RO")
habitat_string <- stri_sub(soil_nmds_mat[[1]], 1, 2)

# make a column with the extracted habitat names and a column with the host/soil repeated
...
## Repeat above steps for all hosts using rarefied root physeq objects to start!
## the same habitat_string (6 "AK" and 6 "RO") applies for all except Kolea which has 5 AK and 5 RO)
...
# merge all host/soil tables by ASV row name, adding NAs where there are no matches
...
# take only ASV count columns
nmds_byplot_abun <- nmds_plot_rbind[,4:ncol(nmds_plot_rbind)]

# convert to matrix
nmds_byplot_abun <- as.matrix(nmds_byplot_abun)

set.seed(123)
# make NMDS
nmds_byplot <- metaMDS(nmds_byplot_abun, distance = "bray")

# extract NMDS scores
byplot_data_scores <- as.data.frame(scores(nmds_byplot))

# add sample column
byplot_data_scores$Sample <- nmds_plot_rbind$Sample

# add habitat column
byplot_data_scores$Habitat <- nmds_plot_rbind$Habitat


# create plot
nmds_byplot_plot <- ggplot(byplot_data_scores, aes(x = NMDS1, y = NMDS2)) + 
    geom_point(data = byplot_data_scores, size = 4, aes(shape = Habitat, colour = Sample))+ 
    theme(axis.text.y = element_text(colour = "black", size = 12, face = "bold"), 
    axis.text.x = element_text(colour = "black", face = "bold", size = 12), 
    legend.text = element_text(size = 12, face ="bold", colour ="black"), 
    legend.position = "right", axis.title.y = element_text(face = "bold", size = 14), 
