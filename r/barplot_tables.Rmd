...
# group by Sample (species) and family
...
ak_unique_table_df <- ak_unique_table_df %>% group_by(Sample, family) %>%
  summarise(count=round((length(unique(ASV))/nrow(ak_skookag_boo) * 100), digits=2)) %>%
  dcast(family ~ Sample) %>%
...
ro_unique_table_df <- ro_unique_table_df %>% group_by(Sample, family) %>%
  summarise(count=round((length(unique(ASV))/nrow(ro_skookag_boo) * 100), digits=2)) %>%
  dcast(family ~ Sample) %>%
  column_to_rownames('family') %>%
