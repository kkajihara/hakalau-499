...
I want to make 2 venns and nestedness, 1 each for AK and ak habitats.
Take rarefied root physeq objects + soil object.
Use subset_samples to keep only AK or ak plots.
...
#Add grass
ak_skookapg_mat <- merge(ak_skookap_mat, ak_grass_mat, by="row.names", all=T)
rownames(ak_skookapg_mat) <- ak_skookapg_mat[,1]
ak_skookapg_mat[,1] <- NULL
...
# Changes NAs to 0
ak_skookag_mat_pa[is.na(ak_skookag_mat_pa)] <- 0

# Makes presence absence (turns any value > 0 into 1)
ak_skookag_mat_pa[ak_skookag_mat_pa > 0] <- 1


ak_skookag_mat$sum <- rowSums(ak_skookag_mat)
t_ak_skookag <- t(ak_skookag_mat)
t_ak_skookag <- as.data.frame(t_ak_skookag)

t_ak_sko <- as.data.frame(t(ak_sko_mat))

which(rowSums(ak_skookag_mat)==0)

t_ak_skooka <- as.data.frame(t(ak_skooka_mat))

ak_skooka_mat$sum <- rowSums(ak_skooka_mat)

 filter(ak_skookag_mat, sum==0)

```

## Convert Presence Absence to Percentages (didn't work)

```{r}

# rows to factors

ak_skookag_mat_pct <- ak_skookag_mat_pa

ak_factors <- apply(ak_skookag_mat_pct, 1, paste, collapse="/")

ak_skookag_mat_pct$Factor <- factor(ak_factors, labels=1:length(unique(ak_factors)))


ak_skookag_mat_pct <- ak_skookag_mat_pct %>% nest(data=c(Soil, `Grass root`, `Koa root`, `Kolea root`, `Ohia root`, `Olapa root`))

ak_skookag_mat_pct <- ak_skookag_mat_pct %>% mutate(hosts = sapply(data, function(x) paste0(sort(x), collapse="-")))

ak_skookag_mat_pct <- ak_skookag_mat_pct %>%
  rownames_to_column('ASV') %>%
  gakup_by(Factor, ASV) %>% 
  summarise(n=n())

ak_skookag_mat_pct$pct <- ak
...
# another way to convert binary to boolean
#ak_skookag_boo[ak_skookag_hosts] = ak_skookag_boo[ak_skookag_hosts] == 1 
...
ak_skookap_euler <- euler(ak_skookap_mat)
ak_skookap_euler_plot <- plot(ak_skookap_euler, fills=TRUE, quantities=TRUE)
ak_skookap_euler_plot

```


## Testing venn accuracy

```{r}

# Make sure ASVs are row names
#ak_skookap_df <- t(ak_skookap_mat)
# Convert matrix to data frame
ak_skookap_df <- as.data.frame(ak_skookap_df)

# Get names of all ASVs present in akala
# Venn says 18, what is it actually?
# This command will take a data frame but not a matrix
ak_some_present <- rownames(ak_skookap_df)[
                        ak_skookap_df$Soil==1 &
                        ak_skookap_df$`Koa root`==0 &
                        ak_skookap_df$`Ohia root`==0 &
                        ak_skookap_df$`Olapa root`==0 &
                        ak_skookap_df$`Kolea root`==1 &
                        ak_skookap_df$`Pilo root`==0 &
                        ak_skookap_df$`Akala root`==1 &
                        ak_skookap_df$`Grass root`==1]


ak_all_filtered_physeq <- prune_taxa(ak_all_present, unrarefied_physeq)
...
sort(rowSums(ak_skookag_mat_nest))

# Ordered by size
ak_sample_order <- c("Soil",
                     "Kolea root",
                     "Grass root",
                     "Akala root",
                    "Olapa root",
...
# Ordered by what looks to be more nested visually
...
ak_rank_scores <- apply(alt_ak_nest_mat, MARGIN=2, function(x) sum(x * rev(seq_along(x))))
