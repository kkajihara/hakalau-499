...
# save for later
ak_skookag_bin <- ak_skookag_mat_pa

ak_skookag_bin <- rownames_to_column(ak_skookag_bin, "ASV")

ak_skookag_bin$family <- ak_tax_table_df$Family[match(ak_skookag_bin$ASV, ak_tax_table_df$ASV)]

ak_skookag_bin <- column_to_rownames(ak_skookag_bin, "ASV")

# convert binary to boolean
ak_skookag_boo <- apply(ak_skookag_bin[1:7], 2, function(x) as.logical(x))
...
## ComplexUpset with Family Annotation (not tested yet)
